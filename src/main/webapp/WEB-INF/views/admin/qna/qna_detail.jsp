...
<script>
	function send(f) {
		...
		f.action = "admin_qna_res_modify.do";
		f.method = "post";
		f.submit();
	}
	function send2(f) {
					...
					<div id="qna_res_form_box">
						...
						<form>
							<table>
								...
								<tr>
									<th width=15%>답변내용</th>
									<td colspan="3"><textarea id="qna_res" name="qna_res"
											rows="5" cols="70" style="resize: none;">${vo.qna_res}</textarea></td>
								</tr>
								<tr>
									<td colspan="4" style="background: none"><input
										type="hidden" name="qna_idx" value="${vo.qna_idx}">
										<button type="button" class="btn1" id="qna_res_btn"
											onclick="send(this.form);">수정하기</button></td>
								</tr>
							</table>
						</form>
