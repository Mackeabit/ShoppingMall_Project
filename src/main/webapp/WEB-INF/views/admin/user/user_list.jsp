...
<head>
<meta charset="UTF-8">
<link rel="stylesheet" href="resources/css/member/member_list.css?after">
...
<script>
    ...
    jQuery(document).ready(function() {
  	...
  	function send(){
  		...
  		location.href="admin_search_user.do?menu=" + menu +" &word=" + word;
  	}
  	
  	function view(idx){
  		location.href = "admin_user_view.do?idx=" + idx;
  	}
</script>
</head>
      		...
      		<table>
      			...
      			<c:forEach var="vo" items="${list}">
	      			<tr>
      					...
      					<td>${vo.user_date}</td>
      					<td>
      						<c:if test="${vo.user_status eq 0}">기본</c:if>
      						<c:if test="${vo.user_status eq 1}">휴면</c:if>
      						<c:if test="${vo.user_status eq 2}">탈퇴대기</c:if>
      						<c:if test="${vo.user_status eq 3}">탈퇴</c:if>
      					</td>
						<c:if test="${vo.user_status ne 3}">
      						<td>
      							<button type="button" style="cursor:pointer" onclick="view(${vo.user_idx});">
      								상세보기
      							</button>
      						</td>
      						<td>
      							<button type="button" style="cursor:pointer" onclick="location.href='admin_user_delete.do?user_idx=${vo.user_idx}'">
      								회원삭제
      							</button>
      						</td>
						</c:if>
