...
<meta charset="UTF-8">
<link rel="stylesheet"
	href="${ pageContext.request.contextPath }/resources/css/review/review_product_list2.css?after">
					...
					<div id="review_box">
						...
						<div id="btn_box">
							<button class="btn" type="button"
								...
								전체 리뷰 목록으로</button>
							<button class="btn" type="button"
								onclick="location.href='admin_review_product_list.do?pd_nm=${list.get(0).getPd_nm()}&standard=${standard }'">
								이전 페이지로</button>
						</div>
						...
						<table>
							...
							<c:forEach var="vo" items="${list }">
								<tr>
									...
									<td>${vo.pd_nm}</td>
									<td><img class="active"
										src="/loc_img/${ vo.pd_img }"></td>
									<td>${vo.user_id }</td>
									<td>${vo.rv_title }</td>
									<td>${fn:split(vo.rv_date, ' ')[0]}</td>
									<td><button type="button"
											onclick="location.href='admin_review_detail.do?rv_idx=${vo.rv_idx}'">
											상세보기</button></td>
								</tr>
							</c:forEach>
						</table>
