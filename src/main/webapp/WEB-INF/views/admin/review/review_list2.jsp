...
<meta charset="UTF-8">
<link rel="stylesheet"
	href="${ pageContext.request.contextPath }/resources/css/review/review_list2.css?after">
...
<script>
	...
	function send(f) {
		f.action = "admin_review_list2.do";
		f.method = "post";
		f.submit();
	}
</script>
				...
				<div id="container_box">
					...
					<div id="review_box">
						...
						<div id="review_select">
							<form>
								<c:if test="${standard eq '평점높은상품순'}">
									<select name="standard">
										<option>최신순</option>
										<option selected>평점높은상품순</option>
										<option>평점낮은상품순</option>
										<option>리뷰많은상품순</option>
										<option>리뷰적은상품순</option>
									</select>
								</c:if>
								<c:if test="${standard eq '평점낮은상품순'}">
									<select name="standard">
										<option>최신순</option>
										<option>평점높은상품순</option>
										<option selected>평점낮은상품순</option>
										<option>리뷰많은상품순</option>
										<option>리뷰적은상품순</option>
									</select>
								</c:if>
								<c:if test="${standard eq '리뷰많은상품순'}">
									<select name="standard">
										<option>최신순</option>
										<option>평점높은상품순</option>
										<option>평점낮은상품순</option>
										<option selected>리뷰많은상품순</option>
										<option>리뷰적은상품순</option>
									</select>
								</c:if>
								<c:if test="${standard eq '리뷰적은상품순'}">
									<select name="standard">
										<option>최신순</option>
										<option>평점높은상품순</option>
										<option>평점낮은상품순</option>
										<option>리뷰많은상품순</option>
										<option selected>리뷰적은상품순</option>
									</select>
								</c:if>
								<input type="button" value="조회" onclick="send(this.form);">
							</form>
						</div>
						...
						<table>
							...
							<c:forEach var="vo" items="${list }">
								<tr>
									...
									<td>${vo.rv_count }</td>
									<td>
									<button type="button" onclick="location.href='admin_review_product_list.do?pd_nm=${vo.pd_nm}&standard=${standard}'">
										상세보기</button>
									</td>
								</tr>
							</c:forEach>
